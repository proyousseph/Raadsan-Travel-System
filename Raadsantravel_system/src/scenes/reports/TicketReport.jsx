...
import { ticketReport } from "../../api/financeapi";
...
const Form = () => {
  ...
  const handleFormSubmit = async (event) => {
    ...
    post.append("action", "TicketReport");
    post.append("FromDate", salesinfo.fromDate);
    post.append("ToDate", salesinfo.toDate);
    var salesreportInfo = await ticketReport(post);
  ...
  const downloadPdf = () => {
    ...
    doc.text("TICKET INFORMATION REPORT", 60, 55);
    const tableOptions = {
      startX: 20,
      startY: 65,
  ...
  return (
    <Box m="20px">
      <ToastContainer />
      <Header title="TICKET REPORT" subtitle="" />
